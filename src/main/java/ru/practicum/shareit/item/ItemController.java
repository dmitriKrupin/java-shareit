...
public class ItemController {
    ...
    // рассмотренных далее.
    @PostMapping
    public Item addItem(@RequestBody ItemDto itemDto, @RequestHeader HttpHeaders header) {
        String userId = header.getFirst("X-Sharer-User-Id");
        log.info("Получаем POST запрос к эндпойнту /items");
        Item item = ItemMapper.INSTANCE.toModel(itemService.addItem(itemDto, userId));
        return item;
    }
    ...
    public Item updateItem(
        ...
        Item item = ItemMapper.INSTANCE.toModel(itemService.updateItem(itemId, updateItemDto, userId));
        return item;
    }
    ...
    public Item getItemById(@PathVariable long itemId) {
        ...
        return ItemMapper.INSTANCE.toModel(itemService.findItemDtoById(itemId));
    }

    @GetMapping
    public List<Item> getAllItemByUserId (@RequestHeader HttpHeaders header) {
        String userId = header.getFirst("X-Sharer-User-Id");
        log.info("Получаем GET запрос к эндпойнту /items");
        return ItemMapperList.INSTANCE.toModelList(itemService.findAllItemByUserId(Long.parseLong(userId)));
    }
    ...
    public List<Item> getItemsBySearch(@RequestParam String text) {
        ...
        return ItemMapperList.INSTANCE.toModelList(itemService.getItemsBySearch(text));
    }
}